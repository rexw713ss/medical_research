...
| 項目 | 狀態 | 正式位置 |
|---|---|---|
| MIMIC leakage-free SOFA 與 6/12/24 h labels | 完成 | `sofa_scores_hourly.csv` |
...
| 新版 FNN full-cohort 6 h training | 進行中 | `outputs/explicit_temporal_fnn_formal_6h/seed_42/` |
| Equal-sample 新版比較、正式消融與最終 external validation | 待完成 | `docs/experiment_status.md` |
...
# 5. 目前執行中：以最佳 validation 參數進行 full-cohort training
& .\outputs\explicit_temporal_fnn_tuning_6h\train_with_best_params.ps1

# 6. Observation-window sensitivity
